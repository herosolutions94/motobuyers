...
function VideoModal({ onClose, videoUrl }) {
  return (
    <div className="steps__modal-overlay" onClick={onClose}>
      <div className="steps__modal steps__modal--wide !p-[0]" onClick={(e) => e.stopPropagation()}>
        <button className="steps__modal-close" onClick={onClose} aria-label="Close">✕</button>
        <div className="video__wrapper">
          <video src={videoUrl} controls autoPlay style={{ width: "100%", borderRadius: "8px" }} />
        </div>
      </div>
    </div>
  );
...
function buildBikeIdRow(formState, sessionId) {
  ...
  return {
    ...
    form_state: JSON.stringify({
      tab: formState.tab, vin: formState.vin,
      year: formState.year, make: formState.make,
      model: formState.model, vinModel: formState.vinModel,
      customMake: formState.customMake,
      manualYear: formState.manualYear, manualMake: formState.manualMake,
    }),
  };
}
...
export default function HeroSection({ content }) {
  ...
  // Field errors
  const [errors, setErrors] = useState({ year: "", make: "", model: "", customMake: "" });

  const vinValid          = isValidVin(vinValue);
  const isUnsupportedYear = manualYear &&
  ...
  const buildFormStateObj = useCallback(() => {
    ...
    return {
      tab:              "manual",
      vin:              vinValue,               // preserve VIN if user decoded then switched
      vehicleIdentified: vinDecoded.vehicleIdentified,
      year:             vinDecoded.year,        // preserve VIN decode results
      make:             vinDecoded.make,
      vinModel:         vinDecoded.vinModel,
      model:            model,
  ...
  }, [activeTab, vinValue, vinDecoded, model, customMake, manualYear, manualMake]);
  ...
  const doSubmit = useCallback(async () => {
    ...
    submitting, activeTab, vinValid, vinDecoding, vinDecodeSuccess,
    manualYear, manualMake, customMake, model, isUnsupportedYear,
    buildFormStateObj, router,
  ]);
                ...
                <div id="steps__page" style={{ minHeight: "unset", background: "none", paddingBottom: 0 }}>
                  ...
                  <div className="steps__tabs">
                    <button
                      ...
                      type="button"
                      onClick={() => { setActiveTab("vin"); setErrors({ year: "", make: "", model: "", customMake: "" }); }}
                    >
                      VIN
                    </button>
                    <button
                      ...
                      type="button"
                      onClick={() => { setActiveTab("manual"); setVinDecodeError(""); }}
                    >
                      Year and Make
                    </button>
                  </div>
                  ...
                  {activeTab === "manual" && (
                    <div className="steps__tab-content">
                      ...
                      <CustomSelect
                        value={manualYear}
                        onChange={(v) => { setManualYear(v); setErrors((p) => ({ ...p, year: "" })); }}
                      ...
                      {!isUnsupportedYear && (
                        <>
                          ...
                          {manualMake === "other" && (
                            <>
                              <label className="steps__field-label !mb-[0]" style={{ marginTop: "1rem" }}>Enter Make</label>
                              <input
                                type="text"
                                className="steps__input"
