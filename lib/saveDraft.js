...
function buildPartialRow(currentPageId, formData, sessionId, extraMeta = {}) {
  ...
  switch (currentPageId) {
    ...
    case "bike-id": {
      ...
      // VIN tab    → year/make/model come from VIN decode (year, make, vinModel)
      // Manual tab → year/make/model come from isolated manual fields
      ...
      return {
        ...
        // Primary columns — always reflect whichever tab was active
        year:        activeYear,
        make:        activeMake,
        model:       activeModel,
        custom_make: activeCMake,

        // VIN-specific columns — only populated when VIN tab was used
        vin:           isVin ? formData.vin || null           : null,
        vin_decoded:   isVin ? !!formData.vehicleIdentified   : false,
        vin_input:     isVin ? formData.vin || null           : null,
        ...
        // Manual-specific columns — only populated when manual tab was used
 ...
 *   - PATCH if a submission ID already exists (user went back and is re-submitting Step 1)
...
export async function saveDraft(currentPageId, nextPageId, formData) {
  ...
  const partial = buildPartialRow(currentPageId, formData, sessionId, extraMeta);

  // Override current_page to be the DESTINATION step
  partial.current_page = PAGE_MAP[nextPageId] ?? nextPageId;

  // ── Determine method ──────────────────────────────────────────────────────
  // For bike-id: POST only if no submission ID exists yet.
  // If the user went back to bike-id and is proceeding again, PATCH instead.
...
// ─── savePhotos (no-op placeholder — photos uploaded instantly in Step6) ──────
